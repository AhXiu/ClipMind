...
interface CaptureOutboxDao {
    ...
    @Query("UPDATE sync_metadata SET uploadState = 'DECRYPTION_FAILED', updatedAt = :now, lastErrorCode = :errorCode WHERE cardId IN (:ids) AND (uploadState = 'READY' OR uploadState = 'RETRYABLE_ERROR')")
    suspend fun markDecryptionFailed(ids: List<Long>, now: Long, errorCode: String): Int

    @Query("UPDATE sync_metadata SET encryptedClientAnalysis = :encryptedAnalysis, updatedAt = :now, lastErrorCode = NULL WHERE cardId = :id AND encryptedClientAnalysis IS NULL AND (uploadState = 'READY' OR uploadState = 'RETRYABLE_ERROR')")
    suspend fun cacheClientAnalysis(id: Long, encryptedAnalysis: String, now: Long): Int

    @Query("UPDATE sync_metadata SET uploadState = 'RETRYABLE_ERROR', retryCount = retryCount + 1, nextRetryAt = :nextRetryAt, updatedAt = :now, lastErrorCode = :errorCode WHERE cardId = :id AND (uploadState = 'READY' OR uploadState = 'RETRYABLE_ERROR')")
    ...
    @Query("UPDATE sync_metadata SET serverCardStatus = :status, serverLastError = :serverLastError, updatedAt = :now WHERE cardId = :id AND serverCardId = :cardId")
    suspend fun updateServerCard(id: Long, cardId: String, status: String, serverLastError: String?, now: Long): Int

    @Query("UPDATE sync_metadata SET serverLastError = :errorCode, updatedAt = :now WHERE cardId = :id AND serverCardId = :cardId")
